...
def get_role_map_from_df(staff_df):
    ...
    for i, row in df.iterrows():
        ...
        if row["朝可"]: roles.add("Morning")
        role_map[i] = roles
    return role_map

def can_cover_required_roles(staff_list, role_map, constraints):
    # Check Morning/Night constraints
    if sum(1 for s in staff_list if "Night" in role_map[s]) < constraints.get('min_night', 3): return False
    if sum(1 for s in staff_list if "Morning" in role_map[s]) < constraints.get('min_morning', 3): return False
    
    # Original Neko/ABC logic (preserved as requested)
...
def solve_schedule_from_ui(staff_df, holidays_df, days_list, constraints, priority_days=[]):
    ...
    # Estimate minimum monthly demand for soft penalty reference
    min_m = constraints.get('min_morning', 3)
    min_n = constraints.get('min_night', 3)
    daily_min_staff = max(4, min_m, min_n) # Baseline for "surplus" calculation
    
    # Store required offs in array for fast access
    req_offs_arr = req_offs
    total_req_offs = np.sum(req_offs_arr)
    # Total capacity = Sum(NumDays - ReqOffs)
    total_capacity_initial = num_staff * num_days - total_req_offs
    ...
    for d in range(num_days):
        ...
        # Priority Days Logic (Keep as is, just visual grouping)
        current_weekday = days_list[d].weekday()
        
        patterns = day_patterns[d]
        
        # 1. First attempt: Strict filtering
        ...
        # Function to process paths with a set of patterns and constraints
            ...
            for path in paths:
                for pat in patterns_to_use:
                    ...
                    if not can_cover_required_roles(pat, role_map, constraints): 
                         penalty += 50000 # High penalty but solvable if necessary
                        ...
                        if work_mask[s] == 1:
                            ...
                            if new_cons[s] > limit:
                                if new_cons[s] == limit + 1: penalty += 1000
                                    ...
                                    if strict_constraints: violation = True; break
                                    else: penalty += 100000 # Massive penalty for breaking max cons in emergency
                            elif new_cons[s] == limit: penalty += 50
                        else:
                            new_cons[s] = 0; new_offs[s] += 1; new_off_cons[s] += 1
                            if is_weekend and is_seishain[s]:
                                # Only count as "Assigned Weekend Off" if it wasn't a fixed off
                                if fixed_shifts[s, d] != '×':
                                    new_weekend_offs[s] += 1
                                    if new_weekend_offs[s] > 1: penalty += 20000 # Strict limit: Max 1 assigned weekend off per month 
                            if new_off_cons[s] >= 3:
                                penalty += 100
                    ...
                    # Check monthly off requirements
                    # If strictly adhering, we fail if we can't meet req_offs.
                    # In emergency (strict_constraints=False), we allow violation with penalty.
                    
                    if strict_constraints:
                        # Cannot exceed total offs needed (soft check usually, here hard)
                        if np.any(new_offs > req_offs): pass 
                         
                        if np.any(new_offs + days_left < req_offs): continue
                    else:
                        # Emergency: penalize if impossible to reach req_offs
                        if np.any(new_offs + days_left < req_offs): 
                             penalty += 100000
                    
                    expected = req_offs * ((d+1)/num_days)
                    # STRICT HOLIDAY ADHERENCE: Increase penalty significantly
                    holiday_diff = np.sum(np.abs(new_offs - expected))
                    penalty += holiday_diff * 200 # increased from 10
                    
                    # Approaching end of month: massive penalty if off-track
                    if days_left < 5:
                         # If it's mathematically impossible to reach target, penalty is applied via 'strict_constraints' check or emergency penalty.
                         # usage: req_offs vs new_offs + days_left
                         # Here we just penalize ANY deviation from ideal curve heavily to force convergence
                         penalty += holiday_diff * 1000
                    
                    
                    # --- Dynamic Resource Conservation Penalty ---
                    # Calculate remaining capacity for this specific path
                    
                    worked_days = (d + 1) - new_offs
                    remaining_capacity = (num_days - req_offs_arr) - worked_days
                    total_remaining_capacity = np.sum(np.maximum(0, remaining_capacity)) # sum of positive remaining
                    
                    # Remaining demand
                    days_remaining_after_today = num_days - 1 - d
                    min_needed = days_remaining_after_today * daily_min_staff
                    
                    tightness = 0
                    if total_remaining_capacity > 0:
                        tightness = min_needed / total_remaining_capacity
                    else:
                        tightness = 2.0 # Critical
                        
                    # Adaptive Penalty
                    surplus_penalty_weight = 500 # Increased Base from 50 to 500 to discourage non-priority usage
                    if tightness > 0.9: surplus_penalty_weight = 1000
                    if tightness > 1.0: surplus_penalty_weight = 5000
                    
                    # --- Priority Day Logic & Late-Month Efficiency ---
                    current_wd_str = weekdays_jp[days_list[d].weekday()]
                    is_priority = current_wd_str in priority_days
                    
                    if is_priority:
                         # PRIORITY DAY: We WANT surplus.
                         # No penalty for surplus. Actually, slight bonus? 
                         # Let's just set penalty to 0 to allow natural filling.
                         surplus_penalty_weight = 0
                    else:
                        # NON-PRIORITY: Save resources for priority days/end of month
                        # Aggressive Late-Month Efficiency
                        if d >= 20: 
                            surplus_penalty_weight = 10000
                    
                    surplus_staff = max(0, len(pat) - daily_min_staff)
        ...
        # Try Normal Beam Search
        next_paths = expand_paths(current_paths, use_patterns, strict_constraints=True)
        
        # Fallback: If no paths found, try relaxed constraints (Emergency Mode)
        if not next_paths:
            # Use ALL patterns, not just valid ones + top 50 invalid
            # And relax constraints
            next_paths = expand_paths(current_paths, patterns[:300], strict_constraints=False) # Try top 300 random patterns with relaxed rules
        
        # If STILL no paths (very rare), try to just carry over previous paths with dummy assignment (everyone off? or random?)
        # For now, let's assume relaxed constraints will find *something* (e.g. everyone works, ignoring limits).
        
        if not next_paths:
            # Absolute last resort: just copy current paths and assign NO ONE for today (incomplete day)
            # This ensures we don't return None.
            for path in current_paths:
                new_sched = path['sched'].copy() # Day d remains 0s (Off)
                # Update constraints as if everyone is OFF
                new_cons = np.zeros(num_staff, dtype=int) # Reset cons
                new_offs = path['offs'] + 1
                new_off_cons = path['off_cons'] + 1
                next_paths.append({
                     'sched': new_sched, 'cons': new_cons, 'offs': new_offs, 
                     'off_cons': new_off_cons, 'weekend_offs': path['weekend_offs'], 
                     'score': path['score'] + 1000000 # Huge penalty
                })
...
with st.form("settings_form"):
    ...
    # --- 基本設定 (New) ---
    with st.expander("🛠️ 基本設定 (必要な人数・優先曜日)", expanded=False):
        c1, c2, c3 = st.columns(3)
        min_morning_req = c1.number_input("朝の必要人数", min_value=1, max_value=10, value=3)
        min_night_req = c2.number_input("夜の必要人数", min_value=1, max_value=10, value=3)
        
        # Priority Days
        default_priority = ["土", "水", "金", "日"]
        weekdays_jp_list = ["月", "火", "水", "木", "金", "土", "日"]
        priority_days_select = c3.multiselect("優先的に確保する曜日", weekdays_jp_list, default=default_priority)
...
if st.button("シフトを作成する"):
    with st.spinner("AIがシフトパズルを解いています...🧩"):
        try:
            constraints = {
                'min_morning': min_morning_req,
                'min_night': min_night_req,
                'priority_days': priority_days_select
            }
            result_df = solve_schedule_from_ui(
                ...
                days_list, 
                constraints,
                priority_days_select # Added priority_days_select as an argument
            )
